...
def generate_crops_dataset(file_path="data/crops.csv", num_crops=15):
    ...
    if num_crops > len(sample_crop_bases):
        print(
            f"Warning: Requested {num_crops} crops, but only {len(sample_crop_bases)} unique base types available. Using all available."
        )
    ...
    for i, base in enumerate(selected_bases):
        ...
        sell_price = base["sell_base"] + random.randint(
            -base["sell_base"] // 10, base["sell_base"] // 10
        )
        seed_cost = base["seed_base"] + random.randint(
            -base["seed_base"] // 10, base["seed_base"] // 10
        )
        crop_yield = (
            base["yield_base"] + random.randint(0, 1)
            if base["yield_base"] > 1
            else base["yield_base"]
        )
