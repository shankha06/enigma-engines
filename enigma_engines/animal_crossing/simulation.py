# --- 5. Simulation Loop ---
from enigma_engines.animal_crossing.agent import SocialEconomistAgent
...
def run_simulation(days_to_simulate):
    console.print(Panel("[bold magenta]--- Starting ACNH Social Economist Agent Simulation --- :rocket:", title="Simulation Start", expand=False))
    ...
    for day in range(days_to_simulate):
        ...
        header_text.append(f"Bells: {format_bell_count(current_env_state['bells'])} | ", style="bold")
        header_text.append(f"Nook Miles: {format_nook_miles(current_env_state['nook_miles'])} | ", style="bold")
        header_text.append(f"Avg. Friendship: {format_friendship(current_env_state['avg_friendship'])}", style="bold")

        day_info = [header_text]

        if current_env_state["turnip_buy_price"] > 0:
            day_info.append(f":moneybag: Daisy Mae selling turnips at: [bold yellow]{current_env_state['turnip_buy_price']}[/bold yellow] bells")
        if current_env_state["turnip_sell_price"] > 0:
            day_info.append(f":chart_with_upwards_trend: Nooklings buying turnips at: [bold green]{current_env_state['turnip_sell_price']}[/bold green] bells")

        if current_env_state['active_nook_tasks']:
            tasks_table = Table(title=":scroll: Available Nook Tasks", show_header=True, header_style="bold blue")
            tasks_table.add_column("Task Name", style="dim", width=30)
            tasks_table.add_column("Miles", justify="right")
            tasks_table.add_column("Duration (Days)", justify="right")
            for task_name, details in current_env_state['active_nook_tasks'].items():
                tasks_table.add_row(task_name, str(details.get('miles', 'N/A')), str(details.get('duration_days', 'N/A')))
            day_info.append(tasks_table)
        else:
            day_info.append(":information_source: No Nook Tasks available today.")


        villager_table = Table(title=":house_with_garden: Villager Friendship Levels", show_header=True, header_style="bold purple")
        ...
        console.print(Panel(Padding("\n".join(str(item) for item in day_info) if not isinstance(day_info[0], Table) and not isinstance(day_info[0], Text) else Columns(day_info, expand=True, equal=True), (1,2)), title=day_panel_title, border_style="cyan"))
        ...
        if day % 100 == 0 or day == days_to_simulate - 1:
            progress_text = Text.assemble(
                (f"Day {env.current_day} ({final_state_today['current_date']}): ", "bold"),
    ...
    console.print(Panel("[bold magenta]--- Simulation Ended --- :checkered_flag:", title="Simulation Complete", expand=False))
    final_state = env.get_state()

    summary_table = Table(title=f":bar_chart: Final State after {days_to_simulate} days", show_header=True, header_style="bold yellow")
    ...
    summary_table.add_row("Total Nook Miles", format_nook_miles(final_state['nook_miles']))
    summary_table.add_row("Average Villager Friendship", format_friendship(final_state['avg_friendship']))
    
    console.print(summary_table)

    villager_final_table = Table(title=":busts_in_silhouette: Final Villager Friendships", show_header=True, header_style="bold cyan")
...
if __name__ == "__main__":
    console.print(Panel(Text("Welcome to the Animal Crossing Simulator!", justify="center"), style="bold green on black"))
    try:
        # Example: Simulate for a small number of days to see rich output
        run_simulation(days_to_simulate=5)
